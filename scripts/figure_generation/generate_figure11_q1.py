#!/usr/bin/env python3
"""
Figure 11 — Representative SWCC Predictions (Set 1: coarse-textured soils)
...
Layout: 2 rows × 1 column (vertical stack, shared x-axis)
  (a) Sand (idx=16, sand ≈ 88 %)
  (b) Silt loam (idx=2, silt ≈ 76 %)

Each panel shows: Observed | GB | VGParamNet (Run B) | MonotonicPINN
Uses VGParamNet Run B predictions from results_pinn_fixed/vgparamnet/run_B/
...
# Colors  (consistent with Figure 10)
...
def main() -> int:
    ...
    # Load predictions — all from the same test ordering
    y_vg   = np.load(ROOT / "results_pinn_fixed/vgparamnet/run_B/theta_vgparamnet_test.npy").astype(np.float32)
    y_gb   = np.load(ROOT / "results_pinn_fixed/vgparamnet/run_B/theta_vgparamnet_test.npy")  # placeholder until GB loaded

    # Load GB via BaselineModels (fast retrain)
    ...
    idx_sand = int(np.where((sand > 70) & (clay < 15))[0][0])
    idx_silt = int(np.where((silt > 50) & (clay < 27))[0][0])

    xmin = float(psi.min())
    xmax = float(psi.max())

    # Verify VGParamNet is Run B
    rmse_vg = float(np.sqrt(np.mean((y_true - y_vg) ** 2)))
    print(f"VGParamNet RMSE (should be ~0.083): {rmse_vg:.4f}")
    print(f"Panel (a): idx={idx_sand}  sand={sand[idx_sand]:.0f}%")
    print(f"Panel (b): idx={idx_silt}  silt={silt[idx_silt]:.0f}%")
    ...
    _plot_swcc(ax_a, psi,
               y_true[idx_sand], y_gb[idx_sand], y_vg[idx_sand], y_pinn[idx_sand],
               f"(a) Sand  (sand = {sand[idx_sand]:.0f}%)",
               bottom=False, xmin=xmin, xmax=xmax)

    _plot_swcc(ax_b, psi,
               y_true[idx_silt], y_gb[idx_silt], y_vg[idx_silt], y_pinn[idx_silt],
               f"(b) Silt loam  (silt = {silt[idx_silt]:.0f}%)",
               bottom=True, xmin=xmin, xmax=xmax)

    fig.align_ylabels([ax_a, ax_b])
    fig.subplots_adjust(left=0.12, right=0.98, top=0.97, bottom=0.10)

    stem = OUTPUT_DIR / "Figure11_Representative_SWCCs_Set1"
